...
*Amended 2026-08-24 — see the Run 1b entry below. The 1555u box and the naming of Serif
Devanagari as the outlier were computed before any `size-adjust` and do not survive it. The
rest of this entry stands.*

**2026-08-24 — Run 1b settled: one 1412.3u line box across all six families.**
`size-adjust` from measured ink, with Latin's x-height (536u) as the target: Sans Devanagari
86.17%, Serif Devanagari 85.90%, Sans Gujarati 88.74%, Serif Gujarati 87.58%. All six then
share a 1412.3u (1.4123em) line box. `--line-height: 1.5`, unitless, applied to every script;
Latin reads slightly airy at that value, which is the accepted trade recorded on 2026-08-18.
Measure stays in `ch` and so varies about 2.8% between scripts — character count per line is
what a measure is for, and pinning one pixel value would impose one script's rhythm on the
other two for no gain at that spread.

**Amends the typo-metrics entry above.** That entry named Serif Devanagari the outlier at a
1555u box. The figure came from raw typo metrics, before any `size-adjust`. Once each Indic
face is scaled down to match Latin's x-height, its ascent and descent scale with it, so the
governing values are Latin's own ascent (1069u) and Serif Gujarati's scaled deep mark
(343.3u). Serif Devanagari is not the outlier. The reasoning was sound; it was applied one
step too early.

**Measured in Chrome 151 on macOS.** Canvas rasterisation is engine- and version-specific, so
the pixel values are not reproducible bit-for-bit elsewhere. `/specimen/` carries the
measurement rig; `scripts/gen-run1b-metrics.py` regenerates the font data, and
`src/data/generated/` is gitignored because it is derived.

**Recorded because it looked like a defect and is not.** All four Indic faces measure exactly
896u above baseline on the `tall` slot. Four typefaces returning an identical figure looked
like a clamped measurement and was challenged twice — remeasured with the overrides
neutralised, and confirmed by ink widths differing per face (752/708/765/761px), proving
...
- [ ] **Where IAST comes from.** Every collection with a Devanagari headword also carries an
      ...
      Either a source supplies it, or the owner transliterates and something checks him, or
      it is generated by rule from the Devanagari. Undecided, and it is the same class of
      risk as the Sanskrit rule: a wrong transliteration is plausible-looking and the owner
      cannot yet catch it. Blocks the first reference entry, not the design runs.
